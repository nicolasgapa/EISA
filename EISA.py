...
def upload(date, receiver_name):
    ...
    :param days_before: How many days before today.
    :param receiver_name: Receiver name (i.e. RX1).
    :return:
    """
    # Identify the directory to the plots that will be uploaded.
    summary_plots_dir = cwd + filesep + os.pardir + filesep + "EISA_OUTPUT" + filesep + receiver_name + filesep + \
                        "GRAPHS" + filesep + date + filesep + "Summary_Plots"

    # Copy the plots to the UPLOAD folder (located inside EISA_OUTPUT).
    copy_tree(summary_plots_dir, cwd + filesep + os.pardir + filesep + "EISA_OUTPUT" + filesep + "UPLOAD" + filesep +
              date[:4] + filesep + date[:6] + filesep + date)
