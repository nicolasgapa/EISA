...
def plot_scintillation_detections(file, graph_type, prn, threshold, location, signal_type_name, date, time_period=1):
    ...
    ax = plt.gca()
    new_array = [np.array([i] * 2) for i in detections]
    new_array = np.stack(new_array, axis=0).T
    detection_map = ax.matshow(new_array, cmap='Oranges', origin='lower', alpha=0.3,
    ...
    cbar.ax.set_yticklabels(['No Scintillation', 'Low', 'High', 'Extreme', 'Multi-Path'])
